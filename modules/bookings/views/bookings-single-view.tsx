...
import BookingPageTagManager from "@bookph/core/app-store/BookingPageTagManager";
import type { getEventLocationValue } from "@bookph/core/app-store/locations";
import {
  getSuccessPageLocationMessage,
  guessEventLocationType,
} from "@bookph/core/app-store/locations";
import { getEventTypeAppData } from "@bookph/core/app-store/utils";
...
import {
  getCalendarLinks,
  CalendarLinkType,
} from "@bookph/core/features/bookings/lib/getCalendarLinks";
import {
  RATING_OPTIONS,
  validateRating,
} from "@bookph/core/features/bookings/lib/rating";
import { isWithinMinimumRescheduleNotice as isWithinMinimumRescheduleNoticeUtil } from "@bookph/core/features/bookings/lib/reschedule/isWithinMinimumRescheduleNotice";
import type { nameObjectSchema } from "@bookph/core/features/eventtypes/lib/eventNaming";
import { getEventName } from "@bookph/core/features/eventtypes/lib/eventNaming";
import { shouldShowFieldInCustomResponses } from "@bookph/core/lib/bookings/SystemField";
import { APP_NAME } from "@bookph/core/lib/constants";
import {
  formatToLocalizedDate,
  formatToLocalizedTime,
  formatToLocalizedTimezone,
} from "@bookph/core/lib/dayjs";
...
import isSmsCalEmail from "@bookph/core/lib/isSmsCalEmail";
import { markdownToSafeHTML } from "@bookph/core/lib/markdownToSafeHTML";
import { getEveryFreqFor } from "@bookph/core/lib/recurringStrings";
import {
  getIs24hClockFromLocalStorage,
  isBrowserLocale24h,
} from "@bookph/core/lib/timeFormat";
import {
  getTimeShiftFlags,
  getFirstShiftFlags,
} from "@bookph/core/lib/timeShift";
...
import { EmailInput, TextArea } from "@bookph/ui/components/form";
...
export default function Success(props: PageProps) {
  ...
  const attendeeTimeZone = bookingInfo?.attendees.find(
  ...
  const tz = props.tz
    ? props.tz
    : isSuccessBookingPage && attendeeTimeZone
      ? attendeeTimeZone
      : timeZone();
  ...
  const icsLink = calendarLinks.find(
    (link) => link.id === CalendarLinkType.ICS
  )?.link;
  ...
  const googleCalendarLink = calendarLinks.find(
    (link) => link.id === CalendarLinkType.GOOGLE_CALENDAR
  )?.link;
  ...
  const eventNameObject = {
    attendeeName: bookingInfo.responses.name as
      | z.infer<typeof nameObjectSchema>
      | string,
    ...
    eventDuration: dayjs(bookingInfo.endTime).diff(
  ...
  const userIsOwner = !!(
    session?.user?.id && eventType.owner?.id === session.user.id
  );
  ...
  const isCancelled =
    ...
    status === "REJECTED" ||
    (!!seatReferenceUid &&
      !bookingInfo.seatsReferences.some(
  ...
  useEffect(() => {
    ...
    setIs24h(
      props?.userTimeFormat
        ? props.userTimeFormat === 24
        : !!getIs24hClockFromLocalStorage()
    );
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  useEffect(() => {
    setCalculatedDuration(
      dayjs(bookingInfo.endTime).diff(dayjs(bookingInfo.startTime), "minutes")
    );
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  ...
  function getTitle(): string {
    ...
    if (bookingInfo.user) {
      const isAttendee = bookingInfo.attendees.find(
  ...
  const needsConfirmationAndReschedulable =
  ...
  const canCancelOrReschedule =
    !eventType?.disableCancelling || !eventType?.disableRescheduling;
  ...
  const isRescheduleDisabled =
    !canReschedule || isWithinMinimumRescheduleNotice;
                ...
                {!isFeedbackMode && (
                  <>
                    <div
                      className={classNames(
                        isRoundRobin &&
                          "relative mx-auto h-24 min-h-24 w-32 min-w-32"
                      )}
                    >
                      ...
                      <div
                        className={classNames(
                          ...
                          !giphyImage &&
                            isReschedulable &&
                            (needsConfirmation || isAwaitingPayment)
                            ? "bg-subtle"
                            : "",
                      ...
                      >
                        ...
                        {(needsConfirmation || isAwaitingPayment) &&
                          isReschedulable && (
                            <Icon
                              name="calendar"
                              className="text-emphasis h-5 w-5"
                            />
                          )}
                        {isCancelled && (
                          <Icon
                      ...
                      <div className="border-subtle text-default mt-8 grid grid-cols-3 gap-x-4 border-t pt-8 text-left sm:gap-x-0 rtl:text-right">
                        ...
                        {previousBooking && (
                          <>
                            ...
                            <div className="col-span-2 mb-6 last:mb-0">
                              <p className="wrap-break-word">
                                {previousBooking?.rescheduledBy}
                              </p>
                              <Link
                                className="text-sm underline"
                                href={`/booking/${previousBooking?.uid}`}
                              >
                                {t("original_booking")}
                              </Link>
                            </div>
                            ...
                            <div className="col-span-2 last:mb-0">
                                ...
                                <div className="mb-3">
                                  ...
                                  {!bookingInfo.eventType
                                    ...
                                    <p
                                      ...
                                      data-testid="booking-host-email"
                                    >
                                      {bookingInfo?.userPrimaryEmail ??
                                        bookingInfo.user.email}
                                    </p>
                              ...
                              {bookingInfo?.attendees.map((attendee) => (
                                ...
                                >
                                  {attendee.name && (
                                    <p
                                      data-testid={`attendee-name-${attendee.name}`}
                                    >
                                      {attendee.name}
                                    </p>
                                  )}
                                  {attendee.phoneNumber && (
                                    <p
                                      data-testid={`attendee-phone-${attendee.phoneNumber}`}
                                    >
                                      {attendee.phoneNumber}
                                    </p>
                                  )}
                                  {!isSmsCalEmail(attendee.email) && (
                                    <p
                                      data-testid={`attendee-email-${attendee.email}`}
                                    >
                                      {attendee.email}
                                    </p>
                                  )}
                        ...
                        {rescheduledToUid ? (
                          <RescheduledToLink
                            rescheduledToUid={rescheduledToUid}
                          />
                        ) : null}
                        ...
                        {!!utmParams && canViewHiddenData && (
                          <>
                            <div className="mt-9 pr-2 font-medium sm:pr-0">
                              {t("utm_params")}
                            </div>
                                    ...
                                    <ul className="stack-y-1 list-disc p-1 pl-5 sm:w-80">
                                      {Object.entries(utmParams)
                                        ...
                                        .map(([key, value]) => (
                                          <li
                                            key={key}
                                            className="text-muted space-x-1 text-sm"
                                          >
                                            <span>{key}</span>:{" "}
                                            <span>{value}</span>
                                          </li>
                          ...
                          <div className="text-default pt-8 text-center text-xs">
                            ...
                            <form
                              onSubmit={(e) => {
                                ...
                                router.push(
                                  `https://cal.com/signup?email=${target.email.value}`
                                );
                              }}
                              className="mt-4 flex"
                            >
